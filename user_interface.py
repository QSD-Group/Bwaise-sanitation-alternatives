...
def main(urine_inputs, feces_inputs, construction_cost, operating_cost, tech_construction_emissions, tech_operating_emissions, correlation_distributions, correlation_parameters, n_samples, discount_rate):
    # import user interface parameters from input spreadsheet
    parameters = pd.DataFrame.transpose(pd.read_excel('Bwaise_sanitation_inputs.xlsx', sheet_name = 'user_interface').set_index('parameters'))
