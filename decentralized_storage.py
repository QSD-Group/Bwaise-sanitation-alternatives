...
def main(excreta_inputs, urine_inputs, feces_inputs, direct_emissions, correlation_distributions, correlation_parameters, n_samples, rate_constant, maximum_methane_emission, CH4_GWP, N2O_GWP, number_users):
    # import module parameters from input spreadsheet
    parameters = pd.DataFrame.transpose(pd.read_excel('Bwaise_sanitation_inputs.xlsx', sheet_name = 'decentralized_storage').set_index('parameters'))
