...
def main(excreta_inputs, urine_inputs, feces_inputs, tech_operating_emissions, operating_cost, correlation_distributions, correlation_parameters, n_samples, rate_constant, maximum_methane_emission, CH4_GWP, N2O_GWP, exchange_rate, discount_rate, number_users, previous_storage_time):
    # import module parameters from input spreadsheet
    parameters = pd.DataFrame.transpose(pd.read_excel('Bwaise_sanitation_inputs.xlsx', sheet_name = 'conveyance').set_index('parameters'))
